...
CImageReaderPNG::CImageReaderPNG(io::IFileSystem* fs)
{
    FileSys = fs;
}
//--------------------------------------------------------------------------------------------------------
CImageReaderPNG::~CImageReaderPNG()
{
    //dtor
}
//--------------------------------------------------------------------------------------------------------
bool CImageReaderPNG::isSupported(const char* file_extension)
{
...
//--------------------------------------------------------------------------------------------------------
bool CImageReaderPNG::isSupported(io::IFile* file)
{
    png_byte header[8];
...
//--------------------------------------------------------------------------------------------------------
renderer::IImage* CImageReaderPNG::LoadImage(const char* path)
{
    io::IFile* png_file = FileSys->open_file(path);
...
//--------------------------------------------------------------------------------------------------------
renderer::IImage* CImageReaderPNG::LoadImage(io::IFile* file)
{
...
//--------------------------------------------------------------------------------------------------------
renderer::IImage* CImageReaderPNG::LoadPNG(io::IFile* file)
{
    ...
    if(!is_png)
    {
        io::Log::println("Failed to load image - corrupted data[not png]",ELL_DEBUG_APP);
        return NULL;
    }
    ...
    if(!png_ptr)
    {
        io::Log::println("Failed to load image - libpng internal error",ELL_DEBUG_APP);
        return NULL;
    }
    ...
    return new renderer::CImage(PixelFomat,core::dimension2du(width,height),data);
}
//--------------------------------------------------------------------------------------------------------
}
